...
import {
  getLatestFailureSequence,
  type FailureSequenceSnapshot,
...
function PortfolioPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      supabase
        .from("child_profiles")
        .select("id, name, age, talents, interests, pdf_unlocked, xp, time_pressure")
        .eq("id", profileId)
        .eq("user_id", userId)
        .maybeSingle(),
      supabase
  ...
  useEffect(() => {
    ...
    try {
      setGentleDismissed(
        localStorage.getItem(`genizio_dismissed_gentle_proposal_${profileId}`) === "1",
      );
    } catch (err) {
  ...
  const acceptGentleProposal = async () => {
    ...
    try {
      ...
      toast.success(
        `${TIME_PRESSURE_LABELS.gentle} activé — ${child.name} a plus de temps pour ses défis.`,
      );
    } catch (err) {
  ...
  useEffect(() => {
    ...
    ensureHypothesesForChild({ data: { childId: profileId } })
      .then((res) => {
        if (res.generated) refetchOpenCycle();
      })
      .catch((err) => {
  ...
  const portraitDomains = [...domainCounts.entries()].sort((a, b) => b[1] - a[1]).slice(0, 6);
    ...
    <div className="min-h-dvh bg-surface pb-24 text-ink ">
      ...
      <main className="mx-auto max-w-6xl px-6 py-10 md:flex ">
        ...
        <div className="min-w-0 flex-1 space-y-6">
              ...
              >
                ...
                <div className="min-w-0 flex-1 w-full">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <p
                      className={`text-[11px] font-extrabold uppercase tracking-widest ${guild.color} opacity-70`}
                    >
                  ...
                  <h2
                    className={`font-display text-balance text-2xl font-black leading-tight ${guild.color}`}
                  >
                    {guild.name}
                  </h2>
                  <p className={`text-xs font-medium italic mt-0.5 ${guild.color} opacity-80`}>
                    « {guild.tagline || guild.description} »
                  </p>
                  ...
                  <div className="mt-3">
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <span className="text-[10px] text-ink/60 font-bold">
                        {nextXP > 0
                          ? `encore ${nextXP} XP pour le niveau ${level + 1}`
                          : "Niveau Max !"}
                      </span>
                    </div>
                ...
                <ChevronRight
                  className={`size-5 shrink-0 opacity-60 ${guild.color} hidden sm:block`}
                />
              </Link>
            );
          })()}
          ...
          {openCycle?.parent_narrative && (
            <div className="rounded-3xl border border-amber-200 bg-amber-50/90 p-6 shadow-md backdrop-blur-md">
              <div className="mb-3 flex items-center gap-3">
                <NayaAvatar
                  size="sm"
                  thoughts={[`Je réfléchis à quelque chose sur ${child.name}...`]}
                />
                <div>
              ...
              <div className="mt-4 flex flex-col  sm:items-center justify-between gap-3 pt-3 border-t border-dashed border-amber-200">
                <p className="text-xs italic text-ink/60">
                  Naya continue d'observer les prochains défis de {child.name} pour affiner sa
                  compréhension.
                </p>
                <Link
          ...
          {child.time_pressure === "standard" &&
            gentleSuggestion?.suggested &&
            !gentleDismissed &&
            (() => {
              const domain = gentleSuggestion.domains[0];
              const domainNote =
                domain && domain !== "domaine inconnu" ? `, surtout dans ${domain}` : "";
              return (
                <div className="rounded-3xl border border-sky-200 bg-sky-50/90 p-6 shadow-md">
                  ...
                  <p className="text-sm font-medium leading-relaxed text-ink">
                    ...
                    propose d'activer le <strong>temps généreux</strong> — les chronos durent alors
                    nettement plus longtemps. C'est votre décision.
                  </p>
          ...
          {failureSequence?.hasSequence && failureSequence.narrative && (
            <div className="rounded-3xl border border-emerald-200 bg-emerald-50/90 p-6 shadow-md">
              <div className="mb-3 flex items-center gap-3">
                <NayaAvatar
                  size="sm"
                  thoughts={[`Je comprends mieux comment apprend ${child.name}...`]}
                />
                <div>
          ...
          {(() => {
            ...
            const getInterestBucket = (interestName: string) => {
              ...
              if (score >= 70)
                return {
                  label: "CONFIRMÉ",
                  cls: "bg-emerald-100 border-emerald-500 text-emerald-800",
                };
              if (score >= 40)
                return {
              ...
              if (score >= 1)
                return { label: "SIGNAL PRÉCOCE", cls: "bg-sky-100 border-sky-500 text-sky-800" };
              return {
                label: "PAS ENCORE EXPLORÉ",
                cls: "bg-stone-105 border-stone-400 text-stone-600",
              };
            };
                ...
                <div>
                  ...
                  <div className="flex justify-between items-start gap-4 mb-4">
                    <div>
                      ...
                      <h2 className="font-display text-balance text-2xl font-black text-ink mt-0.5">
                        {rank}
                      </h2>
                    </div>
                    ...
                    <div className="size-14 rounded-full border border-dashed border-ink/20 bg-surface flex flex-col items-center justify-center shrink-0">
                      <span className="text-[9px] font-black uppercase tracking-wider text-ink/60 leading-none">
                        Level
                      </span>
                      <span className="font-display text-balance text-base font-black text-ink leading-none mt-0.5">
                        {level}
                      </span>
                    </div>
                  </div>
                  ...
                  <ul className="space-y-3 my-4">
                    {childInterests.map((interest: string) => {
                      ...
                      return (
                        <li
                          key={interest}
                          className="flex items-center justify-between py-1 border-b border-stone-100 last:border-b-0"
                        >
                          <span className="text-xs font-bold text-ink/75">{interest}</span>
                          <span
                            className={`rounded-full border px-3 py-0.5 text-[9px] font-black uppercase tracking-wider ${bucket.cls}`}
                          >
                            {bucket.label}
                          </span>
                        </li>
          ...
          {(() => {
            const coveredDomains = new Set(
              (child.interests ?? [])
                .map(
                  (tag) =>
                    Object.entries(INTERESTS_BY_TALENT).find(([, v]) => v.tags.includes(tag))?.[0],
                )
            ...
            return (
              <div className="rounded-3xl border border-emerald-200 bg-emerald-50/90 p-6 shadow-md">
                ...
                <p className="text-sm font-medium leading-relaxed text-ink">
                  {child.name} a beaucoup approfondi son côté <strong>{label}</strong> à travers ses
                  défis — nettement plus que ce que ses centres d'intérêt déclarés laissaient
                  penser.
                </p>
            ...
            <div className="rounded-3xl border border-red-300 bg-red-50/70 p-5 shadow-sm flex flex-wrap items-center gap-3">
              ...
              <div className="flex-1 min-w-48">
                <p className="text-sm font-black text-red-800">
                  Accès expiré — renouvelez pour continuer les défis
                </p>
              ...
              <div className="flex items-start gap-4">
                ...
                <div>
                  <div className="flex flex-wrap items-center gap-2">
                    <h3 className="font-display text-balance text-lg font-black text-ink">
                  ...
                  <p className="text-xs font-semibold text-ink/70 mt-1 leading-relaxed max-w-xl">
                    {enrolledSeason.description ||
                      "Un parcours immersif pour développer de nouvelles compétences."}
                  </p>
                  {(enrolledSeason as any).individual_start_date &&
                    (enrolledSeason as any).individual_end_date && (
                      <div className="mt-3 flex flex-wrap items-center gap-4 text-xs font-bold text-ink/60 bg-white/50 p-2 px-3 rounded-xl border border-emerald-100">
                        ...
                        <div className="flex items-center gap-1.5 text-emerald-800">
                          ...
                          Fin prévue :{" "}
                          {new Date((enrolledSeason as any).individual_end_date).toLocaleDateString(
                            "fr-FR",
                          )}
                        </div>
                      </div>
                  ...
                  {(() => {
                    ...
                    const daysLeft = Math.ceil(
                      (new Date(rawEnd).getTime() - Date.now()) / (24 * 60 * 60 * 1000),
                    );
                ...
                <div className="rounded-3xl border border-ink/10 bg-amber-50 p-6 shadow-xl flex flex-col  items-center justify-between gap-6">
                  <div className="flex items-start gap-4">
                    ...
                    <div>
                      <div className="flex flex-wrap items-center gap-2">
                        <h3 className="font-display text-balance text-lg font-black text-ink">
                          Le Passeport d'Excellence Génizio
                        </h3>
                        <span
                      ...
                      <p className="text-xs font-semibold text-ink/70 mt-1 leading-relaxed max-w-xl">
                        Un document d'orientation certifié conçu pour appuyer les dossiers de
                        candidature aux lycées d'élite et universités. Il compile sa Carte des
                        Talents, ses 10 meilleurs défis réalisés (avec photos de preuves) et les
                        observations de Naya.
                      </p>
                    </div>
                  </div>
          ...
          {child.talents && Object.values(child.talents).some((v) => v > 0) && (
            <div className="rounded-3xl border border-ink/10 bg-white p-6 shadow-xl">
              ...
              <p className="mb-5 text-xs font-medium leading-relaxed text-ink/60">
                Les possibilités restent infinies — {child.name} pourrait aussi bien concevoir,
                enseigner, chercher ou entreprendre. On garde toutes les portes ouvertes.
              </p>
              ...
              const completedSubforms = Array.from(
                new Set(
                  completed
                    ...
                    .filter(
                      (sub): sub is string =>
                        typeof sub === "string" && Boolean(TALENT_SUBFORM_OPPORTUNITIES[sub]),
                    ),
                ),
              );
              ...
              return (
                <div className="rounded-3xl border border-brand/20 bg-gradient-to-br from-brand/5 via-white to-sky/5 p-6 shadow-xl space-y-4">
                  ...
                  <p className="text-xs font-medium leading-relaxed text-ink/70">
                    Pistes d'exploration d'avenir basées sur les sous-domaines de compétences
                    spécifiques analysés lors des défis de {child.name}.
                  </p>
          ...
          <div className="rounded-3xl border border-ink/10 bg-white p-6 shadow-xl space-y-6">
            <div>
              ...
              <p className="text-xs font-semibold text-ink/60 mt-1">
                Les talents de {child.name} se développent sous forme de cartes d'aptitudes
                évolutives adaptées à sa tranche d'âge.
              </p>
            </div>
                  ...
                  >
                    <div>
                      ...
                      <div className="flex justify-between items-center mb-3">
                        <span
                          className={`rounded-md border border-ink/10 px-2 py-0.5 text-[8px] font-black uppercase tracking-wider ${card.tagClass}`}
                        >
                          {card.typeLabel}
                        </span>
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <h4 className="font-display text-balance text-sm font-black text-ink leading-none">
                          {details.title}
                        </h4>
                      </div>
                  ...
                  {topDomains.map(([domain, count], i) => (
                    <div key={domain} className="flex items-center gap-3">
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          <span
                            className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${getDomainStyle(domain)}`}
                          >
                            {domain}
                          </span>
                        ...
                        <div className="h-2 bg-ink/6 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.round((count / (topDomains[0][1] || 1)) * 100)}%`,
                            }}
                          />
                        </div>
                      </div>
                  ...
                  return (
                    <div key={monthKey}>
                      ...
                      >
                        ...
                        {!isDefaultExpanded && (
                          <ChevronDown
                            className={`size-3.5 text-ink/40 transition-transform ${isExpanded ? "rotate-180" : ""}`}
                          />
                        )}
                      </button>
                              ...
                              <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="mt-1.5 flex flex-wrap items-center gap-2">
                                    <span
                                      className={`text-[10px] font-bold px-2.5 py-0.5 rounded-full border ${getDomainStyle(c.domain)}`}
                                    >
                                      {c.domain}
                                    </span>
                                    {c.completed_at && (

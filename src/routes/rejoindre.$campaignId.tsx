...
import { getCampaignPublicInfo, enrollChildViaCampaignLink, type CampaignPublicInfo } from "@/lib/campaigns.functions";
    ...
    return (
      <div className="min-h-dvh flex items-center justify-center bg-surface px-6">
        <div className="max-w-md text-center">
          ...
          <Link to="/" className="mt-6 inline-block text-sm font-bold text-brand underline">Retour à l'accueil</Link>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-dvh bg-surface px-4 sm:px-6 py-10 sm:py-16">
      <div className="mx-auto max-w-md">
        ...
        <div className="rounded-3xl border border-ink/10 bg-white p-6 sm:p-8 shadow-xl">
          ...
          <h1 className="font-display text-2xl sm:text-3xl font-black text-ink text-balance">{campaign.name}</h1>
          ...
          <div className="mt-6 pt-6 border-t border-ink/10">
            {enrolledChildName ? (
              <div className="text-center space-y-3">
                ...
                <h2 className="font-display text-lg font-black text-ink">Inscription confirmée !</h2>
              ...
              <p className="text-center text-sm font-medium text-ink/60">
                Ce programme a atteint sa capacité maximale. Contactez l'organisation partenaire pour plus d'informations.
              </p>
            ) : !campaign.isWithinWindow ? (
              <p className="text-center text-sm font-medium text-ink/60">
                Ce programme n'est pas actif pour le moment (période : {new Date(campaign.startDate).toLocaleDateString("fr-FR")} → {new Date(campaign.endDate).toLocaleDateString("fr-FR")}).
              </p>
            ) : sessionLoading ? (
              <div className="flex justify-center py-4"><GenizioLoader /></div>
            ) : !session ? (
              <div>
                <p className="text-sm text-ink/70 mb-4">Connectez-vous pour inscrire votre enfant à ce programme.</p>
                <Link
                  to="/auth"
              ...
              <div className="text-center">
                <p className="text-sm text-ink/70 mb-4">Créez d'abord le profil de votre enfant pour l'inscrire à ce programme.</p>
                <button
              ...
              <div>
                <p className="text-xs font-bold uppercase tracking-wide text-ink/50 mb-2">Quel enfant inscrire ?</p>
                <div className="space-y-2 mb-4">
                  {children.map((c) => (
                    <label
                      key={c.id}
                      className={`flex items-center gap-3 rounded-2xl border-2 px-4 py-3 cursor-pointer transition-all ${
                        selectedChildId === c.id ? "border-brand bg-brand/5" : "border-ink/10 hover:border-ink/20"
                      }`}
                    >
                      <input
                        type="radio"
                      ...
                      />
                      <span className="text-sm font-bold text-ink">{c.name} <span className="text-ink/40 font-medium">({c.age} ans)</span></span>
                    </label>
                  ))}
                </div>
                <button
      ...
      {isCreatingProfile && session && (
        <ProfileDialog
          ...
          onSaved={() => { setIsCreatingProfile(false); loadChildren(); }}
        />
      )}
    </div>
  );

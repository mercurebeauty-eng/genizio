...
describe("diagnostic-hypotheses", () => {
  ...
  it("calculateInformationGain maximise l'IG pour le contexte discriminant", () => {
    const hyp = formulateHypothesis("c1", "leadership_coordination", "individuel_autonome", 0.5, "collectif_equipe");
  ...
  it("calculateInformationGain retourne 0 si l'hypothèse est déjà résolue", () => {
    const hyp = formulateHypothesis("c1", "leadership_coordination", "individuel_autonome", 0.9, "collectif_equipe");
  ...
  it("updateHypothesisWithEvidence augmente la confiance en cas de succès", () => {
    let hyp = formulateHypothesis("c1", "leadership_coordination", "individuel_autonome", 0.5, "collectif_equipe");
  ...
  it("assessTriangulationStatus passe à triangulated si >= 2 contextes et haute confiance", () => {
    let hyp = formulateHypothesis("c1", "transmission_pedagogique", "collectif_equipe", 0.6, "individuel_explicatif");
    
    // Succès en collectif (déjà origine, mais on simule la confirmation)
    hyp = updateHypothesisWithEvidence(hyp, { evidenceId: "e1", context: "collectif_equipe", success: true, weight: 1.0 });
    
    // Succès en explication Naya
    hyp = updateHypothesisWithEvidence(hyp, { evidenceId: "e2", context: "individuel_explicatif", success: true, weight: 1.0 });
    
    // Succès en tutorat
    hyp = updateHypothesisWithEvidence(hyp, { evidenceId: "e3", context: "collectif_tutorat", success: true, weight: 1.0 });
  ...
  it("assessTriangulationStatus passe à contextualized si succès dans un contexte et échec fort dans un autre", () => {
    let hyp = formulateHypothesis("c1", "creativite_sous_contrainte", "individuel_autonome", 0.8, "collectif_equipe");
    
    // Succès en solo
    hyp = updateHypothesisWithEvidence(hyp, { evidenceId: "e1", context: "individuel_autonome", success: true, weight: 0.8 });
    
    // Échec sous pression collective
    hyp = updateHypothesisWithEvidence(hyp, { evidenceId: "e2", context: "collectif_equipe", success: false, weight: 0.9 });

...
// ---------------------------------------------------------------------------
// CONSTITUTION DE G�0N�0RATION � fragments partagés
// ---------------------------------------------------------------------------
...
export const GENIZIO_PRINCIPLES = `PRINCIPES DE G�0N�0RATION G�0NIZIO (règles d'excellence strictes, à respecter impérativement) :
...
- CHIFFRES ET MESURES R�0ELS OBLIGATOIRES : chaque défi doit faire réaliser à l'enfant au moins une action quantitative ou langagière précise, avec des VALEURS EXACTES écrites dans les étapes (ex: "mesure 5 cm de fil", "calcule le périmètre du potager de 4 m sur 3 m", "compte 12 graines", "chronomètre 45 secondes", "trouve un mot de 4 syllabes") � jamais d'approximations vagues ("un peu de", "plein de", "au hasard"). Ancre ces mesures dans le quotidien local (tissus, ficelle, terrain, marché, eau, distance, temps). Aligne leur difficulté sur ce que les meilleurs systèmes éducatifs du monde attendent de cet âge (méthode Singapour, Common Core US) : l'ambition Génizio est qu'à la sortie, chaque enfant maîtrise au minimum le niveau international de son âge. Ces valeurs réelles sont la matière exacte que Naya nommera dans "academic_secret" (périmètre, masse, durée, fraction...).
- INTERDICTION DU BRICOLAGE PASSIF : ne fais JAMAIS d'un simple assemblage de carton/bouteille le c�ur du défi. Les objets du quotidien (maison ou extérieur : eau, sel, miroir, ficelle, chronomètre, ombres, plantes, architecture du quartier) ne sont que des instruments de mesure ou de laboratoire, pas de la décoration.
- 5 ARCH�0TYPES DE QU�`TES D'�0LITE (alterner rigoureusement d'un défi à l'autre) :
  1. �x� Laboratoire d'Expérimentation & Physique : tester une loi ou une hypothèse mesurable (densité, gravité, réfraction, équilibre, réactions).
  2. �x"�️ Autopsie & Inversion Logique : analyser un texte, une équation, une carte ou un énoncé volontairement piégé par Naya et identifier les anomalies.
  3. �a"️ Ingénierie & Prototype Fonctionnel : construire un mécanisme physique (levier, rampe, poulie, pont) qui résout un problème précis sous contrainte de ressources.
  ...
  5. �x�:️ Plaidoyer & Stratégie Sociale : construire une argumentation structurée avec 3 preuves pour mener une enquête ou défendre une position lors d'un mini-débat.
- PR�0COCIT�0 GUID�0E (Méthode Singapour) : ne te contente pas de vérifier passivement les acquis basiques de l'âge de l'enfant. Propose un défi qui introduit un concept du niveau supérieur (N+1), tout en le rendant manipulable et compréhensible par l'action concrète.
- CENTRES D'INT�0R�`T = LEVIERS COMPORTEMENTAUX ET MODES COGNITIFS PROFONDS : Ne traite jamais un centre d'intérêt comme un simple thème ou un hobby décoratif (ex: "football", "dinosaures"). Décode et exploite le LEVIER COMPORTEMENTAL ET LE MODE OP�0RATOIRE MENTAL sous-jacent de l'enfant (ex: "Démonte pour comprendre", "Négocie toujours", "A besoin de bouger pour réfléchir"). Utilise ces traits comme M�0CANIQUE ET POSTURE D'APPRENTISSAGE. Si l'enfant "démonte pour comprendre", propose un défi de déconstruction/analyse inverse. Chaque défi doit employer la mécanique d'action préférée de l'enfant (démonter, schématiser, simuler, optimiser, enquêter).
- HARMONIE INT�0RIEUR & EXT�0RIEUR : alterne entre le laboratoire de la maison et le terrain d'investigation extérieur (jardin, cour, quartier, parc, architecture locale) selon le sujet.
...
- CLART�0 POUR L'ENFANT : le titre et la description doivent être compréhensibles directement par l'enfant de cet âge, sans qu'un adulte ait besoin de les lui expliquer. �0vite le jargon technique ou adulte � si un mot technique est indispensable, explique-le simplement dans la même phrase.`;
...
export const ACADEMIC_REFERENTIAL_INSTRUCTION = `R�0F�0RENTIEL ACAD�0MIQUE (calibrage international, pas une échelle maison) : ce référentiel n'est pas une moyenne locale ni un niveau inventé par l'application � il est calé sur les standards des meilleurs systèmes éducatifs du monde (Common Core US, Singapore Math, NGSS, SHAPE America, CASEL, NFEC � �0tats-Unis, Singapour, Chine). L'ambition Génizio : chaque enfant atteigne au minimum le niveau international attendu pour son âge, dans chaque domaine. Si le défi relève d'un des domaines ci-dessous, détermine "academic_domain" ("mathematiques" | "langage" | "sciences" | "corporelle" | "sociale" | "emotionnelle" | "entrepreneuriale" | "artisanale" | "spatiale"), "academic_level_age" (nombre entier = l'âge auquel correspond R�0ELLEMENT le contenu du défi que tu viens de concevoir, d'après ce référentiel international � PAS forcément l'âge de l'enfant), et "academic_reference_note" (1 phrase courte citant la ligne précise du référentiel/standard international sur laquelle tu t'es basé, ex: "toutes les tables à un chiffre mémorisées vers 8 ans" � pas juste "niveau 8 ans"). Pour "creative" (créativité pure, imaginaire libre) ou tout domaine hors de cette liste, omets les trois champs.

MATH�0MATIQUES / LOGIQUE :
...
// �0tendue le 2026-08-10 (décision #59) : l'encart devient un lien direct entre le
...
export const ACADEMIC_SECRET_INSTRUCTION = `SECRET ACAD�0MIQUE DE NAYA ("academic_secret") : Génère obligatoirement un paragraphe captivant de 4 à 6 phrases à destination de l'enfant, en quatre temps :
...
export const AGE_DEVELOPMENT_GUIDANCE = `CONSIGNES DE D�0VELOPPEMENT LI�0ES ì L'�GE :
...
export const KIND_GUIDANCE_INSTRUCTION = `TYPE DE D�0FI ("kind") ET GUIDAGE ("guidance_level") :
...
// sous-forme physique où le potentiel s'exprime le mieux (endurance �0� explosivité �0�
...
- corporelle : "endurance" (effort prolongé) | "explosivite" (saut, sprint, puissance brève) | "coordination_fine" (précision main/�il) | "coordination_collective" (jeu d'équipe, synchronisation) | "precision" (viser, ajuster, répéter un geste exact)
...
// avait "�0tapes claires (3 à 6)" � aucune indication sur le niveau de granularité,
...
// ---------------------------------------------------------------------------
// DIAGNOSTIC BAY�0SIEN � rappels système du moteur d'hypothèses
// ---------------------------------------------------------------------------
...
R��GLE ABSOLUE : raisonne UNIQUEMENT à partir des données fournies dans le snapshot. Si un signal est absent, ne l'invente pas.
...
DIRECTIVES DE SP�0CIALISTE DOCTORAL (PR�0VENTION DES �0TIQUETTES L�0G��RES) :
...
- Chaque hypothèse cite dans "evidence_log" les n�uds réels du snapshot qui la justifient.
...
Contraintes :
- SYNTH��SE P�0DAGOGIQUE ET APPRENTISSAGE �0QUILIBR�0 : Associe les leviers comportementaux observés par le parent (posture cognitive) avec la cartographie des talents de l'enfant. Les intelligences actuellement les moins explorées chez cet enfant sont ${leastExplored.join(" et ")}. Sauf si le contexte les rend peu réalistes, au moins un des ${count} défis DOIT utiliser la posture ou mécanique d'action préférentielle de l'enfant comme passerelle naturelle pour explorer l'une de ces intelligences moins travaillées � c'est ainsi que Naya révèle des talents cachés en s'appuyant sur ses moteurs d'action naturels.
...
export interface BuildSingleChallengePromptInput {
  ...
  /** "" ou "6. UTILISATION DES MAT�0RIAUX MENTIONN�0S : ..." */
...
Contexte immédiat (TR��S IMPORTANT) :
...
export interface BuildHomeworkPromptInput {
  ...
  zpaRationale: string;
  /** "" ou "- CONTEXTE D'ANXI�0T�0 D�0TECT�0 : ..." */
  anxietyLine: string;
...
Ta mission est de transformer une CONSEIGNE DE DEVOIR SCOLAIRE sous forme d'un D�0FI PHYSIQUE, CAPTIVANT ET CONCRET.

Profil de l'enfant :
- Prénom : ${childName}
- �ge chronologique : ${childAge} ans
- Classe actuelle : ${gradeInfoLabel} (${gradeInfoCycle})
- Ville / pays : ${profileLocation}
- Modes d'engagement et leviers comportementaux observés par le parent :
${interestsPayload}

CONSIGNE DE SOUTIEN SCOLAIRE / DEVOIR ì FUSIONNER :
...
ZPA ET CALIBRAGE DE DIFFICULT�0 :
...
R��GLES DE FUSION ACAD�0MIQUE-LUDIQUE STRICTES :
1. LE DEVOIR DOIT �`TRE R�0ELLEMENT R�0VIS�0/APPRIS : La réussite du défi doit garantir que l'enfant a pratiqué ou assimilé la consigne scolaire ("${homeworkInstruction}"). Le défi ne doit PAS détourner l'enfant du devoir, mais en faire la mécanique centrale du jeu.
...
${pied}"Titre chaleureux et rassurant",
  ...
  "duration": "10 min",
  "steps": ["�0tape 1 très simple", "�0tape 2 très simple"],
...
export interface BuildJustInTimeHintPromptInput {
  ...
  /** �0tape où l'enfant est bloqué (texte brut de l'étape). */
  currentStep: string;
...
�0tape où ${childName} est bloqué·e : « ${currentStep} »

${stepsList}

${interestsLine}

R��GLES ABSOLUES, sans exception :
...
// L'aspiration reste une HYPOTH��SE à explorer : observe les aptitudes réelles, ne
...
Aspiration déclarée : « ${aspirationLabel} »${sourceNote} � C'EST UNE HYPOTH��SE ì EXPLORER, jamais un verdict, jamais une étiquette. ${childName} peut vouloir cela par envie réelle, par mimétisme, ou parce qu'il pense que ça rapporte : ton travail est d'OBSERVER ses aptitudes réelles à travers ce défi, pas de confirmer la déclaration.
...
// M�`ME objectif pédagogique avec une autre manière de le montrer (jusqu'à 3 essais,
...
N'ABANDONNE PAS ce que cet enfant doit apprendre : reformule le M�`ME objectif pédagogique avec une AUTRE manière de le lui montrer.

OBJECTIF P�0DAGOGIQUE (à conserver strictement identique � même compétence, même niveau, jamais plus difficile) :
${input.originalObjective}

MODALIT�0 IMPOS�0E � "presentation_mode": "${input.presentationMode}" : ${semantics}
...
- OVERRIDE (review 2026-08-12, P2) : la clause « PR�0COCIT�0 GUID�0E (N+1) » des principes Génizio ci-dessous NE S'APPLIQUE PAS à une reformulation � difficulté strictement égale ou plus douce (remise en confiance, jamais une épreuve).
...
// ---------------------------------------------------------------------------
// ESPACE D�0COUVERTE � Analyse cognitive et comportementale d'une exploration libre
// ---------------------------------------------------------------------------
...
- Si l'activité montre une capacité supérieure inattendue ou un grand intérêt spontané, formule une HYPOTH��SE sans porter de verdict définitif.

DONN�0ES DE L'EXPLORATION :
...
R��GLES D'ANALYSE :
1. �0value l'initiative, la persévérance, la curiosité et l'autonomie sur une échelle de 1 à 10.

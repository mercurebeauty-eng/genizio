...
export function mapDiscoveryDifficultyToLevelAge(difficulty: string | null | undefined, childAge: number): number {
  switch (difficulty) {
    case "facile": return Math.max(2, childAge - 1);
    case "moyen": return childAge;
    case "difficile": return childAge + 2;
    case "eleve": return childAge + 3;
    default: return childAge;
  }
}
...
export function calibrateDomainCapability(
  ...
  const sorted = [...evidences].sort((a, b) => new Date(a.occurredAt).getTime() - new Date(b.occurredAt).getTime());
  ...
  for (const ev of sorted) {
    ...
    if ((isSuccess || isPartial) && w >= 0.5) { // Un signal assez fort
 ...
 */
export function sampleTargetLevelForChallenge(capability: DomainCapabilityState, randomRoll: number): { targetLevelAge: number; targetRole: ChallengeTargetRole } {
  // 70% de consolidation (N_stable ou N_stable + 1)
  if (randomRoll < 0.70) {
    // Si explore et stable sont identiques, on donne le stable.
    const lvl = capability.exploratoryLevelAge > capability.stableLevelAge && randomRoll < 0.35 
...
export function formatDynamicCapabilityInstruction(capabilities: DomainCapabilityState[]): string {
  ...
  const lines = capabilities.map(cap => {
    ...
    if (cap.stableLevelAge === cap.exploratoryLevelAge && cap.exploratoryLevelAge === cap.peakLevelAge) {

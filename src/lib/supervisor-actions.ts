// Superviseur Copilote (décision #74) — journal d'audit des actions superviseur.
//
// Non-bloquant pour l'appelant (pattern logChallengeOutcome / verifyAndLog) : une erreur
// de journalisation ne doit JAMAIS faire échouer l'action opérateur — elle est déjà
// autorisée, la trace est un filet. Sert aussi de journal de séance : les notes du
// superviseur sont tracées ici (action 'notes') plutôt que dans challenges.notes, qui
// reste le journal d'apprentissage du parent.

export async function logSupervisorAction(params: {
  supervisorUserId: string;
  childId: string;
  challengeId?: string | null;
  action: string;
  payload?: Record<string, unknown>;
}): Promise<void> {
  try {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // (supabaseAdmin as any) : tables superviseur service-role (mêmes tables que
    // supervisors.functions.ts — cast systématique, le client typé ne les connaît
    // que si les types ont été régénérés après la migration).
    await (supabaseAdmin as any).from("supervisor_actions").insert({
      supervisor_user_id: params.supervisorUserId,
      child_profile_id: params.childId,
      challenge_id: params.challengeId ?? null,
      action: params.action,
      payload: params.payload ?? {},
    });
  } catch (err) {
    console.error("supervisor_actions log failed (non-fatal):", err);
  }
}

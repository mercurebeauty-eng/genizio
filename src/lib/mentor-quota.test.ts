...
describe("computeMentorQuota", () => {
  it("une campagne créée avant le cutover garde 5 de base", () => {
    expect(
      computeMentorQuota({ referenceCreatedAt: "2026-07-20T00:00:00.000Z", extraQuota: 0 }),
    ).toBe(5);
  });

  it("une campagne créée après le cutover n'a droit qu'à 1 de base", () => {
    expect(
      computeMentorQuota({ referenceCreatedAt: "2026-08-10T00:00:00.000Z", extraQuota: 0 }),
    ).toBe(1);
  });
  ...
  it("le quota supplémentaire s'ajoute dans la limite du plafond de 5", () => {
    expect(
      computeMentorQuota({ referenceCreatedAt: "2026-07-20T00:00:00.000Z", extraQuota: 3 }),
    ).toBe(5);
    expect(
      computeMentorQuota({ referenceCreatedAt: "2026-08-10T00:00:00.000Z", extraQuota: 3 }),
    ).toBe(4);
  });

...
export function SocialShareBar({ title, url }: { title: string; url?: string }) {
  ...
  const shareUrl = url || (typeof window !== "undefined" ? window.location.href : "https://www.genizio.com/guides");
    ...
    if (navigator.share) {
      try {
        await navigator.share({
          title,
          text: `${title} — À lire sur Génizio :`,
          url: shareUrl,
        });
      } catch {
        // Ignorer l'annulation de l'utilisateur
      }
    } else {
      handleCopy();
    }
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      {/* WhatsApp - Canal n°1 en Afrique Francophone & groupes de parents */}
      <a
        href={whatsappUrl}
        target="_blank"
      ...
      <button
        ...
        aria-label="Copier le lien ou partager"
        className="flex items-center gap-1 rounded-full bg-ink/5 px-2.5 py-1.5 text-xs font-bold text-ink transition-all hover:bg-ink/10 press-brand cursor-pointer"
      >
        {copied ? (
          <>

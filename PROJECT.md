...
| 3 | Hybrid Parent UI & Homework Mode Toggle (R2, R3) | Hybrid input UI (explicit prompt vs CP-3ème suggestions/gaps), mode toggle (Défis Libres vs Devoirs Scolaires), profile integration, and UI component tests. | M1, M2 | DONE |
| 4 | E2E Verification, Vitest Suite & Forensic Audit | `npx tsc --noEmit` (0 errors), dedicated Vitest test suite validation (100% pass), Challenger execution verification, and Forensic Audit (`teamwork_preview_auditor`) CLEAN verdict. | M1, M2, M3 | DONE |


## Interface Contracts

### `AcademicSubject` & `GradeLevel`
- `GradeLevel`: `'CP' | 'CE1' | 'CE2' | 'CM1' | 'CM2' | '6eme' | '5eme' | '4eme' | '3eme'`
- `AcademicSubject`: `'maths' | 'francais' | 'sciences' | 'histoire_geo' | 'anglais'`
- `BehavioralDriver`: `'deconstruire' | 'schematiser' | 'simuler' | 'enqueter'`
...
export interface HomeworkPayload {
  ...
  explicitTask?: string; // e.g. "Tables de 7", "Accord du participe passé"
  suggestedTopicId?: string;
  driver?: BehavioralDriver;
  zpaLevel?: number; // Bayesian difficulty factor [1..5]
}
```

### `NayaChallengePromptContext`
- Concatenates child profile `interests`, grade level, explicit/suggested academic homework notion, matched `BehavioralDriver`, and `zpaLevel` telemetry into Naya system prompt context.

...
- ✅ Audit RLS complet du reste du schéma (2026-07-17) : faille grave corrigée sur `posts` (n'importe quel parent pouvait publier un post public attribué à n'importe quel enfant d'une autre famille, vérifié par attaque/défense réelle), même classe de bug sur le bucket `proofs`/`createOrder`/`generateChallenges`. Mur Public rendu réellement fonctionnel : tables `post_likes`/`comments` + trigger `SECURITY DEFINER`. Voir [[genizio-decisions]] #22. Committé (`1910b4c`).

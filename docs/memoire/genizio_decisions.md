...
**⚠️ Décision PRODUIT, NON implémentée** — posée en session produit (product-intelligence-architect), après constat : la direction de l'app fait du superviseur un copilote, mais le code ne lui donne qu'un dashboard lecture seule + déclaration de séance, et la V4 (PR 82-87) a industrialisé l'économie du superviseur-observateur sans toucher à ses droits. Aucune table `supervisor_actions` ni `supervisor_reports` n'existe en base.

...
## Décision #83 : Vue globale d'activité du mentor (BI) — revenus, paiements, taux, évolution (2026-08-16)

**✅ IMPLÉMENTÉE (2026-08-16, branche `fix/mentor-bi`)** — constat du porteur : « le mentor dans son espace n'a pas de vue globale sur son activité — il ne sait pas combien il gagne, combien il reçoit, ce qui a été payé ou en attente, son taux horaire, le nombre de séances effectuées, l'évolution de ses jeunes suivis ». L'espace `/mentor` n'affichait qu'un chip en-tête (score, séances confirmées du mois, « ≈ X F à venir ») : aucun total, aucune ventilation payé/en attente, aucun historique, aucune évolution par enfant. Réponse : une **Vue d'ensemble** (BI) dans `/mentor`, sans aucune migration — le `payout_xof` snapshot posé à la déclaration et les statuts de `mentor_sessions` suffisent.

1. **Sémantique financière (affichage)** : *gagné* = séances **confirmées** par le parent (`confirmed` + `approved` + `paid`, la déclaration seule ne suffit plus depuis la #79) ; *reçu* = statut `paid` (payé par l'admin) ; *en attente* = `approved` (approuvé, à payer) + `confirmed` (à approuver), montants séparés ; *déclaré non confirmé* = `declared` (en attente du parent, non acquis) ; *contesté* = exclu des gains, compteur affiché. Les sommes sont exactes par construction (snapshot immuable — jamais recalculées).
2. **`src/lib/mentor-activity.ts`** (helpers PURS, testés ×8) : `computeEarningsBreakdown` (ventilation + compteurs par statut, une passe), `sessionsInMonth` (fenêtre mois courant, bornes incluses/exclues), `buildMonthlyActivitySeries` (6 derniers mois, mois sans séance présents en zéro — la courbe ne saute pas).
3. **`getMentorActivityOverview`** (`mentor-activity.functions.ts`, requireSupabaseAuth + bannissement vérifié) : séances du mentor (bornées 2 000, ordre desc — ≤ 5 enfants × 12/mois couvre des années), feedback famille (moyenne globale + par enfant), bilans (dernier par enfant + compteurs par statut), progression des défis par enfant, ponctualité du mois. Payload : earnings (toutes périodes + mois courant), série mensuelle, enfants (séances confirmées total/ce mois, défis complétés/total, statut du bilan, note famille moyenne + nombre de notes), qualité (ponctualité, note moyenne, contestations, bilans par statut).
4. **UI `/mentor`** : bascule segmentée **« Vue d'ensemble · Mes enfants »** sous l'en-tête (**défaut : Vue d'ensemble** — répond directement au constat ; « Mes enfants » conserve 100 % de l'existant à 1 clic). Vue d'ensemble : 6 cartes KPI (Total gagné, Reçu, En attente, **Taux par séance** — recalculé comme à la déclaration via `computeMentorPayoutXof` à partir du palier et du bonus points chargés, soit 3 500 F standard / 3 750 F confiance + 5-10 % points ; séances ce mois / attendues ; enfants suivis), ventilation **Paiements** (payé / approuvé-à-payer / confirmé-à-approuver / déclaré-attente-parent / contesté, avec montant total + ce mois + compteur), graphique recharts **Évolution mensuelle** (barres = séances confirmées, ligne = gains en F, 6 mois), cartes **Évolution des jeunes suivis** (guilde, suivi depuis, séances, défis avec barre de progression, chip bilan, note famille), pastilles **Confiance & qualité** (score, ponctualité, note famille, contestations, bilans en attente/validés, palier, points, statut).
5. **Pas de « taux horaire » en base** : les séances ≈ 1 h, le taux affiché est dérivé du payout actuel (source unique pricing.ts + mentor-score.ts) — aucun champ ni migration nouvelle. **Hors périmètre** : le cycle de paiement (declared → confirmed → approved → paid) n'est pas modifié, seule la visibilité change.
...
 # #   D � c i s i o n   # 8 9   :   E x t e n s i o n   d e   l a   l i m i t e   d ' � g e   d e   1 6   �   2 1   a n s 
 * * D � c i s i o n * *   :   L a   b o r n e   d ' � g e   m a x i m u m   d e   l ' a p p l i c a t i o n   p a s s e   d e   1 6   �   2 1   a n s   ( c o u v e r t u r e   j u s q u ' a u   n i v e a u   C � g e p / S u p � r i e u r ) . 
 * * P o u r q u o i * *   :   D e m a n d e   e x p l i c i t e   d e   l ' u t i l i s a t e u r   p o u r   p e r m e t t r e   l ' a c c o m p a g n e m e n t   d e s   j e u n e s   a d u l t e s   p o s t - b a c   ( l y c � e ,   t e r m i n a l e ,   d � b u t   d ' u n i v e r s i t � ) .   
 * * A p p l i c a t i o n * *   : 
 -   * * B a s e   d e   d o n n � e s * *   :   M o d i f i c a t i o n   d e   l a   c o n t r a i n t e   ` c h i l d _ p r o f i l e s _ a g e _ c h e c k `   ( 5   �   2 1   a n s )   v i a   l a   m i g r a t i o n   ` 2 0 2 6 0 8 2 9 1 3 0 0 0 0 _ u p d a t e _ a g e _ l i m i t _ 5 _ 2 1 . s q l ` . 
 -   * * U I * *   :   ` P r o f i l e D i a l o g . t s x `   a u t o r i s e   d e s   d a t e s   d e   n a i s s a n c e   j u s q u ' �   2 1   a n s   e n   a r r i � r e . 
 -   * * R � f � r e n t i e l   a c a d � m i q u e * *   :   ` M A X _ R E F E R E N T I A L _ A G E `   p a s s e   �   2 1   d a n s   ` s r c / l i b / a c a d e m i c - l e v e l s . t s `   a v e c   a j o u t   d e s   l i b e l l � s   " S u p � r i e u r   /   C � g e p   ( B a c + 1   �   B a c + 4 ) " . 
 -   * * I A   ( N a y a ) * *   :   L e   p r o m p t   s y s t � m e   e s t   m i s   �   j o u r   p o u r   e n g l o b e r   l e s   j e u n e s   a d u l t e s   e t   u n e   n o u v e l l e   t r a n c h e   " 1 7   �   2 1   a n s "   e s t   a j o u t � e   �   ` A G E _ D E V E L O P M E N T _ G U I D A N C E `   a v e c   i n t e r d i c t i o n   s t r i c t e   d ' i n f a n t i l i s e r . 
  
 
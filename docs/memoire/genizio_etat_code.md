...
**Bugs de production antérieurs — CLÔTURÉS (2026-08-13, décision #51)** : (1) « Commencer le défi sans effet visible » — cause racine identifiée : le FILTRE de liste, pas la mutation (le clic mettait bien le défi à `in_progress` en base, mais la mise à jour optimiste retirait la carte du filtre « À faire » sans suivi — seul un toast passait). Correctif du 2026-08-05 déjà en place (le filtre suit la carte vers « En cours »), angles morts vérifiés (filtres all/in_progress, échec serveur → rollback + toast, un seul CTA dans l'app), régression verrouillée par `src/lib/challenge-list-filters.ts` + 5 tests (PR #56). Reste : test navigateur réel. (2) Fix « bouton non réussi à todo » — déjà en production (reframe 2026-08-09, `notCompletedButton` au statut `todo` avec dialog) ; le `git stash` a été résorbé (PR #50 : seule la partie `autoPort` de `.claude/launch.json` restait non commitée).

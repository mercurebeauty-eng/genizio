...
- **[observé]** Bucket `proofs` : `public = true` + policy SELECT **anon** (`20260715181500:8-10`) → les photos de preuve de tous les enfants sont publiques sans authentification. **✅ CHANTIER LIVRÉ (2026-08-15)** : code (upload en path `proofs/{childId}/{file}`, affichage par URLs signées via `src/lib/proof-image.tsx` — `getProofImageSrc`/`useProofImageUrl`/`ProofImage`, gère aussi les anciennes URLs publiques ; surfaces migrées : galerie portfolio, export passeport (fetch + img), modale mentor) + migration `20260815190000_multicouche_proofs_private.sql` (bucket privé, suppression de la policy anon, policy SELECT parent-owner/mentor assigné, backfill URLs→paths). **⚠️ Ordre de déploiement impératif** : déployer le CODE d'abord (merge PR), PUIS `supabase db push` la migration — sinon les images cassent pendant la fenêtre.
...
| **V6 — Supervision** | F + G | **✅ Livrée (2026-08-15)** : artefact `supabase/monitoring/multicouche-checkup.sql` (10 requêtes lentes pg_stat_statements, scans séquentiels, tailles de tables, probes RLS mentor, activité mentor/bilans) ; **probes prod réels** : `compute_executive_kpis` (8 enfants / 75 défis / 22 complétés / actifs 7j=6, 30j=8) et `compute_progression_health` (stale + durée moyenne par domaine) OK via service role ; backfill `parent_profiles` vérifié (3 comptes, email/téléphone/created_at peuplés) ; table `mentor_activation_codes` en place (vide, normal). **`pg_stat_statements` CONFIRMÉ actif en prod** (la vue répond — première exécution Studio) ; colonnes renommées PG14+ (`*_exec_time` au lieu de `*_exec_ms`, corrigé dans le checkup) |

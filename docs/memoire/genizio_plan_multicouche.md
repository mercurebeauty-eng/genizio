...
| **V4 — Pagination** | D1 (9 onglets) + D2 (parent) + D3 (mentor) + C3 (filtres avancés) | **✅ Batch 1 (2026-08-15)** : Exécutif (KPIs en SQL via RPC `compute_executive_kpis` + annuaire paginé 20, fin de `listAllUsers`) ; Mentors (`listMentorsAdmin` paginé par mentor — UUID léger + slice + recherche email SQL) ; Paiements (`listPaymentsAdmin` page/status/count, fin de la troncature à 100 + `parent_profiles`). **✅ Batch 2 (2026-08-15)** : fin de `listAllUsers` dans Commerce/Abonnements/Codes de campagne (→ `parent_profiles`) ; côté parent : filtre `deleted_at` manquant corrigé + bornes (quest 100, portfolio 100, défis 200, `exportUserData` sans soft-supprimés) ; dashboard mentor : borne 200 sur les défis + `resolveChildAccompaniment` parallélisé. **Reste (batch 3)** : pagination SQL des commandes (Commerce), agrégats SQL Naya/Progression/Talents (vues matérialisées si probes > 300 ms) |

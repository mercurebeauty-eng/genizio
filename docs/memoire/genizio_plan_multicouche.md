...
| **V4 — Pagination** | D1 (9 onglets) + D2 (parent) + D3 (mentor) + C3 (filtres avancés) | **✅ Batch 1 livré (2026-08-15)** : Exécutif (KPIs en SQL via RPC `compute_executive_kpis` + annuaire paginé 20, fin de `listAllUsers`) ; Mentors (`listMentorsAdmin` paginé par mentor — UUID léger + slice + recherche email SQL) ; Paiements (`listPaymentsAdmin` page/status/count, fin de la troncature à 100 + `parent_profiles`). **Reste (batch 2)** : Commerce, Naya, Progression, Talents, Abonnements, Codes de campagne, listes parent (défis/portfolio/quest/export) + dashboard mentor |
